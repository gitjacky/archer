...
class InceptionDao(object):
    ...
    def criticalDDL(self, sqlContent):
        ...
        for row in sqlContent.rstrip(';').split(';'):
            if re.match(
                    ...
                    row.lower()):
                result = (
                '', '', 2, '驳回高危SQL', '不能包含【DROP DATABASE】|【DROP TABLE】|【TRUNCATE PARTITION】|【TRUNCATE TABLE】关键字！', row,
                '', '', '', '')
                criticalSqlFound = 1
            else:
                result = ('', '', 0, '', 'None', row, '', '', '', '')
    ...
    def sqlautoReview(self, sqlContent, clusterName, isSplit="no", isBackup="否"):
        '''
            ...
            if criticalDDL_check is not None:
                ...
                if preCheckResult is not None:
                    ...
                    if isSplit == "yes":
                        ...
                        masterUser, masterPassword, masterHost, str(masterPort), sqlContent)

...
def detail(request, workflowId):
    ...
    context = {'currentMenu':'allworkflow', 'workflowDetail':workflowDetail, 'listContent':listContent,'listAllReviewMen':listAllReviewMen,'urole':urole}
...
def searchflow(request):
    ...
    role = loginUserOb.role
    if navStatus == 'all' and role == '审核人':
        searchworkflow = workflow.objects.filter(workflow_name__icontains = wflowname).order_by('-create_time')[offset:limit]
        context = {'currentMenu':'allworkflow','listWorkflow':searchworkflow,'pageNo':pageNo, 'navStatus':navStatus, 'PAGE_LIMIT':PAGE_LIMIT, 'role':role}

    elif navStatus == 'all' and role == '工程师':
        searchworkflow = workflow.objects.filter(workflow_name__icontains = wflowname,engineer=loginUser).order_by('-create_time')[offset:limit]
        context = {'currentMenu':'allworkflow','listWorkflow':searchworkflow,'pageNo':pageNo, 'navStatus':navStatus, 'PAGE_LIMIT':PAGE_LIMIT, 'role':role}
...
def mongowork(request):
    ...
    if navStatus == 'all' and role == '审核人':
        ...
        mogolist = mogocode.objects.filter(mogo_stat=Const.mogostat['waiting'], mogo_submit=loginuser).order_by('-mogo_subtime')[offset:limit]
    elif navStatus == 'finish' and role == '审核人':
        mogolist = mogocode.objects.filter(mogo_stat=Const.mogostat['finish']).order_by('-mogo_subtime')[offset:limit]
    elif navStatus == 'finish' and role == '工程师':
        mogolist = mogocode.objects.filter(mogo_stat=Const.mogostat['finish'], mogo_submit=loginuser).order_by('-mogo_subtime')[offset:limit]
    elif navStatus == 'abort' and role == '审核人':
        mogolist = mogocode.objects.filter(mogo_stat=Const.mogostat['abort']).order_by('-mogo_subtime')[offset:limit]
    elif navStatus == 'abort' and role == '工程师':
        mogolist = mogocode.objects.filter(mogo_stat=Const.mogostat['abort'], mogo_submit=loginuser).order_by('-mogo_subtime')[offset:limit]
    elif navStatus == 'exception' and role == '审核人':
        mogolist = mogocode.objects.filter(mogo_stat=Const.mogostat['exception']).order_by('-mogo_subtime')[offset:limit]
    elif navStatus == 'exception' and role == '工程师':
        mogolist = mogocode.objects.filter(mogo_stat=Const.mogostat['exception'], mogo_submit=loginuser).order_by('-mogo_subtime')[offset:limit]
    else:
        context = {'errMsg': '传入的navStatus参数有误！'}
        return render(request, 'error.html', context)
    ...
    context = {'currentMenu':'mongo', 'mongoworkflow':mogolist,'pageNo':pageNo, 'navStatus':navStatus, 'PAGE_LIMIT':PAGE_LIMIT, 'role':role,'all_reviewer': all_reviewer}
